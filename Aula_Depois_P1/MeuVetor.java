...
public class MeuVetor{
    ...
    public int partition(int p, int r){
        ...
        v[r] = v[i];
        v[i] = x;
        return i; //retorno é a posicao do pivo
    }
    //!quicksort 
    public void quicksort(int p, int r){
        ...
        if (p < r){
            //p - contador inicial
            //q - pivo
            //r - ultimo elemento do vetor
            int q = partition(p, r);
            System.out.println(", q = " + q );
            quicksort(p, q-1); //pega uma posicao anterior do pivo (q-1)
            quicksort(q+1, r); //pega a posicao sucessor do pivo (q+1)
        }
        System.out.println();
    }
}
